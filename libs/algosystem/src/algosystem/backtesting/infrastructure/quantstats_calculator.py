...
class QuantStatsMetricsCalculator(MetricsCalculator):
    ...
    def calculate(self, equity: EquityCurve, benchmark: EquityCurve | None) -> PerformanceMetrics:
    ...
    def time_series(
        self, equity: EquityCurve, benchmark: EquityCurve | None, window: int = 30
    ) -> dict[str, pd.Series]:
    ...
    @staticmethod
    def _benchmark_returns(benchmark: EquityCurve | None) -> pd.Series | None:
    ...
    def _compute(self, key: MetricKey, calculation: Callable[[], object]) -> float | None:
        try:
    ...
    def _annualized_return(self, strategy_returns: pd.Series, equity: EquityCurve) -> float | None:
    ...
    def _downside_deviation(self, strategy_returns: pd.Series) -> float | None:
        downside_returns = strategy_returns[strategy_returns < 0]
        if downside_returns.empty:
            return 0.0
        return self._finite_or_none(downside_returns.std() * np.sqrt(252))

    def _jarque_bera(self, strategy_returns: pd.Series) -> dict[MetricKey, float | None]:
        try:
    ...
    def _monthly_metrics(self, strategy_returns: pd.Series) -> dict[MetricKey, float | None]:
        try:
            monthly_returns = _period_returns(strategy_returns, "ME")
        except Exception:
