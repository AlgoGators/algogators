...
class OverfitResults:
    ...
    def analytical_deflated_sharpe(self, returns: np.ndarray | None = None) -> dict:
        ...
        test_stat = (sr_hat - sr0) * math.sqrt(T - 1) / denom if T > 1 else 0.0
    ...
    def surface_summary(self) -> list[str]:
        ...
        lines.extend(
            [
                ...
                (f"  Frac above 50% of best            : {fh:.4f}  [{rating(fh, 0.3, 0.1)}]"),
                (f"  Plateau score (>70% of best)      : {ps:.4f}  [{rating(ps, 0.2, 0.05)}]"),
                (
