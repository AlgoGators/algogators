...
export default function Backtesting() {
  ...
  const [_availableCustomMetrics, setAvailableCustomMetrics] = useState<
    Array<{
      filename: string;
      name: string;
      description: string;
  ...
  const fetchMetrics = useCallback(
    ...
    [preferences, selectedCategory, dateRange, selectedCustomMetrics]
  ); // Add selectedCustomMetrics to dependency array
  ...
  useEffect(() => {
    if (metrics) {
      ...
      if (timestamps.length > 0) {
        ...
        console.warn(
          "No valid date keys found in metrics. Defaulting to current date."
        );
  ...
  const parseChartData = (data: any, label: string, color: string) => {
    ...
    const labels = validTimestamps.map(
    ...
    <div className="relative">
      ...
      <div className="pt-16">
        <div className="grid grid-cols-2 gap-6">
          ...
          {preferences.percentage_change_vs_Index &&
            metrics?.percentage_change_vs_Index && (
              <Chart
                data={parseChartData(
                  metrics.percentage_change_vs_Index,
