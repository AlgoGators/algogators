...
def _resolve_series(
    item_id: str, provider: DataProvider, start: date, end: date
...
class ComparePane(Vertical):
    ...
    def _run(self) -> None:
        ...
        if (
            item_a in (None, Select.BLANK)
            or item_b in (None, Select.BLANK)
            or analysis in (None, Select.BLANK)
        ):
        ...
        if not isinstance(timeframe, str):
            # Unreachable while the Select has allow_blank=False; narrows NoSelection.
            self.notify("Pick a timeframe.", severity="warning")
            return
    ...
    def _run_analysis(
        self, item_a: str, item_b: str, analysis: str, start: date, end: date
    ) -> None:
        ...
        # (chart function, positional args, kwargs) — the chart functions take
        # different leading arguments, so declare the callable's shape loosely.
        chart_render: tuple[Callable[..., Image.Image], tuple[Any, ...], dict[str, str]] | None = (
            None
        )
        ...
        if analysis == "correlation-matrix":
            ...
            chart_render = (
                line_chart,
                (rolling_correlation(data, label_a, label_b),),
                {"title": f"Rolling corr: {label_a} vs {label_b}"},
            )
            ...
            chart_render = (
                multi_line_chart,
                (relative_performance(data),),
                {"title": "Relative Performance (rebased=100)"},
            )
        elif analysis == "spread":
            chart_render = (
                line_chart,
                (spread_series(data, label_a, label_b),),
                {"title": f"Spread: {label_a} - {label_b}"},
            )
        elif analysis == "ratio":
            chart_render = (
                line_chart,
                (ratio_series(data, label_a, label_b),),
                {"title": f"Ratio: {label_a} / {label_b}"},
            )
