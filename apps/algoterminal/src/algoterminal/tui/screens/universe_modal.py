...
class UniverseModal(ModalScreen[Universe | None]):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="universe-form"):
            ...
            yield Label("Name")
            yield Input(
                value=u.name if u else "", placeholder="e.g. my-basket", id="name", disabled=bool(u)
            )
            yield Label("Symbols (comma-separated)")
            yield Input(
                value=", ".join(u.symbols) if u else "",
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if not (name and symbols_raw):
            ...
            return
        # A cleared Select yields Select.BLANK (NoSelection), not a usable value.
        if not isinstance(asset_class, AssetClass) or not isinstance(source, str):
            self.notify("Asset class and source are required.", severity="error")
            return
