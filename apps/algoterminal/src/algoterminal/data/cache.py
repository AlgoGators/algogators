...
def merge_and_write(
    provider_name: str, symbol: str, existing: pd.DataFrame | None, fresh: pd.DataFrame
) -> pd.DataFrame:
...
def covers_range(df: pd.DataFrame | None, start: date | None, end: date | None) -> bool:
    ...
    if start is not None and df.index.min().date() > start:
        return False
    return not (end is not None and df.index.max().date() < end)


def missing_ranges(df: pd.DataFrame | None, start: date, end: date) -> list[tuple[date, date]]:
