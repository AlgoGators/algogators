...
ARTICLES: dict[str, tuple[str, str]] = {
    ...
    "AMC": (
        "AMC_Entertainment_Holdings",
        "AMC Entertainment — meme-stock cohort alongside GameStop",
    ),
    ...
    "ARTIFICIAL_INTELLIGENCE": (
        "Artificial_intelligence",
        "General public interest in AI as a macro theme",
    ),
    "RECESSION": (
        "Recession",
        "Public search interest in economic recession, a soft sentiment indicator",
    ),
...
class WikiPageviewsProvider(DataProvider):
    ...
    def fetch(
        ...
        if not gaps:
            # missing_ranges() only ever reports "no gaps" for a real cached frame.
            if cached is None:
                raise RuntimeError(f"no cached data for {symbol!r} despite no missing ranges")
