...
def _finish(fig: Figure, ax: plt.Axes, title: str) -> Image.Image:
    ...
    # fig.canvas is typed as FigureCanvasBase, but _figure() always attaches a
    # FigureCanvasAgg — the only backend with buffer_rgba().
...
def line_chart(
    series: pd.Series, title: str = "", width: int = 900, height: int = 400, color: str = ORANGE
) -> Image.Image:
...
def multi_line_chart(
    frame: pd.DataFrame, title: str = "", width: int = 900, height: int = 400
) -> Image.Image:
    fig, ax = _figure(width, height)
    ...
    _date_axis(ax)
    legend = ax.legend(
        loc="upper left", fontsize=8, facecolor=SURFACE, edgecolor=CREAM, framealpha=0.6
    )
...
def equity_curve_chart(
    equity: pd.Series, title: str = "Equity Curve", width: int = 900, height: int = 400
) -> Image.Image:
    return line_chart(equity, title=title, width=width, height=height, color=ORANGE)


def drawdown_chart(
    equity: pd.Series, title: str = "Drawdown", width: int = 900, height: int = 280
) -> Image.Image:
...
def rolling_correlation_chart(corr: pd.Series, width: int = 900, height: int = 320) -> Image.Image:
    return line_chart(
        corr,
        title=str(corr.name or "Rolling Correlation"),
        width=width,
        height=height,
        color=ORANGE,
    )
...
def step_chart(
    series: pd.Series, title: str = "", width: int = 900, height: int = 280, color: str = WARNING
) -> Image.Image:
    ...
    ax.fill_between(
        series.index, series.values, 0, step="post", color=color, alpha=0.25, antialiased=True
    )
    ax.axhline(0, color=CREAM, alpha=0.3, linewidth=0.8)
    _date_axis(ax)
    return _finish(fig, ax, title or str(series.name or ""))
