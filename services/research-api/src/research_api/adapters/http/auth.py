...
from research_api.adapters.serializers.identity import (
...
from research_api.infrastructure.config.dependencies import (
    check_password_hash,
...
def _dev_user_from_config(config):
    try:
        ...
        current_app.logger.warning("[DEV_MODE] DEV_USER_ID is not an integer; falling back to 1")
...
def login():
    ...
    try:
        ...
        current_app.logger.info("Login successful for email: %s, role: %s", email, user.role)
...
def dev_login():
    ...
    except DevLoginUnavailableError:
