...
from research_api.application.shared.errors import NotFoundError
from research_api.domain.portfolio.calculations import (
...
class StrategyDataNotFoundError(NotFoundError):
...
def build_incubating_strategy(row: Mapping[str, Any], now: datetime) -> dict[str, Any]:
    ...
    return {
        ...
        "mock_capital": float(row["mock_capital"]) if row.get("mock_capital") is not None else None,
...
class GetStrategyDetail:
    ...
    def execute(self, strategy_id: str) -> dict[str, Any]:
        ...
        if detail is None:
            raise StrategyDataNotFoundError(strategy_id)
        return detail
...
class ListStrategies:
    ...
    def execute(self) -> list[dict[str, Any]]:
        ...
        for cfg in self.registry.list(active_only=True):
            try:
                latest = self.reader.fetch_summary_row(cfg["strategy_type"], cfg["portfolio_id"])
...
class ListIncubatingStrategies:
    ...
    def execute(self, now: datetime) -> list[dict[str, Any]]:
        return [
            build_incubating_strategy(row, now) for row in self.reader.list_incubating_strategies()
        ]
