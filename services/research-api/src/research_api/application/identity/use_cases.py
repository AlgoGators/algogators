...
from research_api.application.shared.errors import NotFoundError, ValidationError
from research_api.domain.identity.models import User, has_registered_password
from research_api.domain.identity.password_policy import validate_password
...
class EmailNotAuthorizedError(ValidationError):
    """Email is not present in the pre-authorized users table."""


class AccountAlreadyRegisteredError(ValidationError):
...
class DevLoginUnavailableError(NotFoundError):
...
class RegisterUser:
    ...
    def execute(self, email: str, password: str, first_name: str, last_name: str) -> User:
        ...
        if password_error:
            raise PasswordPolicyViolationError(password_error)

        user = self.users.find_by_email(email)
        if not user:
            raise EmailNotAuthorizedError()
        if has_registered_password(user):
            raise AccountAlreadyRegisteredError()

        hashed_password = self.hasher.hash(password)
        return self.users.complete_registration(email, hashed_password, first_name, last_name)
