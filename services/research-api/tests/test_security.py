...
def test_validate_password_helper():
    from research_api.domain.identity.password_policy import MIN_PASSWORD_LENGTH, validate_password
...
def _import_app(env):
    ...
    return subprocess.run(
        [sys.executable, "-c", "import research_api.app"],
        env=full_env,
        capture_output=True,
        text=True,
    )
...
def test_jwt_secret_fail_closed_in_production():
    ...
    env["CORS_ORIGINS"] = "https://research_api.example.com"  # so CORS passes first
...
def test_production_boots_with_valid_config():
    ...
    env["FLASK_ENV"] = "production"
    env["JWT_SECRET_KEY"] = "some-real-secret"
    env["CORS_ORIGINS"] = "https://research_api.example.com"
    result = _import_app(env)
