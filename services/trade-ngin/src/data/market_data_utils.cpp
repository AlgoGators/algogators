...
std::string get_market_data_columns(AssetClass asset_class) {
    switch (asset_class) {
        case AssetClass::EQUITIES:
            ...
            //
            // Equity tables also use different base column names (date/ticker) than
            // futures (time/symbol) -- aliased here too so callers get a uniform
            // time/symbol/open/high/low/close/volume shape regardless of asset class.
            return "date AS time, ticker AS symbol, adj_open AS open, adj_high AS high, "
                   "adj_low AS low, adjusted_close AS close, adj_volume AS volume";
