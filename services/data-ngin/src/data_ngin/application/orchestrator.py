...
from data_ngin.domain.ports import CleanerPort, FetcherPort, RepositoryPort, SymbolSourcePort
from data_ngin.utils.dynamic_loader import determine_date_range, get_instance
from data_ngin.utils.logging_config import setup_logging
...
class Orchestrator:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.loader: SymbolSourcePort = get_instance(
            self.config, "loader", "class", expected_port=SymbolSourcePort
        )
        self.fetcher: FetcherPort = get_instance(
            self.config, "fetcher", "class", expected_port=FetcherPort
        )
        self.cleaner: CleanerPort = get_instance(
            self.config, "cleaner", "class", expected_port=CleanerPort
        )

    async def retrieve_and_process_data(
        self, symbol: dict[str, str], start_date: str, end_date: str
    ) -> bool:
        """
        ...
        inserter: RepositoryPort = get_instance(
            self.config, "inserter", "class", expected_port=RepositoryPort
        )
        batch_config = self.config.get("batch_downloading", {})

        try:
            logging.info(
                f"Fetching raw data for symbol: {symbol['dataSymbol']}, loaded_asset_type: {symbol['instrumentType']}, start: {start_date}, end: {end_date}"
            )
    ...
    async def run(self) -> None:
        ...
        results = await asyncio.gather(
            *[self.retrieve_and_process_data(symbol, start_date, end_date) for symbol in symbols],
        ...
        if failures:
            ...
            logging.error(
                f"Pipeline execution failed for {len(failures)}/{len(symbols)} symbol(s): {summary}"
            )
            raise RuntimeError(
                f"Pipeline execution failed for {len(failures)}/{len(symbols)} symbol(s): {summary}"
            )
