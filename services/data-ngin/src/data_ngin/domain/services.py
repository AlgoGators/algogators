...
from data_ngin.domain.models import BackAdjustment, RollEvent, StalenessReport
...
class SymbolRemapper:
    ...
    DEFAULT_REMAP: ClassVar[dict[str, str]] = {
        "ES": "MES",
        "RTY": "M2K",
        "NQ": "MNQ",
        "YM": "MYM",
    ...
    def __init__(self, remap_table: dict[str, str] | None = None) -> None:
        self.remap_table: dict[str, str] = (
            remap_table if remap_table is not None else dict(self.DEFAULT_REMAP)
        )
...
class BackAdjuster:
    ...
    def detect_rolls(self, data: pd.DataFrame) -> list[RollEvent]:
        ...
        for i in range(1, len(data)):
            if (
                data.loc[i, "symbol"] != data.loc[i - 1, "symbol"]
                and data.loc[i, "volume"] > data.loc[i - 1, "volume"]
            ):
    ...
    def apply(self, data: pd.DataFrame, asset_type: str | None = None) -> pd.DataFrame:
...
class MissingDataFiller:
    ...
    def __init__(self, missing_data_config: dict | None = None) -> None:
...
class StalenessChecker:
    ...
    def check_staleness(
        self, latest_date: str | None, now: datetime | None = None
    ) -> StalenessReport:
        """
        Args:
