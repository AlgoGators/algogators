...
class CSVLoader(Loader):
    ...
    def load_symbols(self) -> dict[str, str]:
        ...
        try:
            ...
            raise ValueError(
                f"The CSV file at {self.contract_path} is empty or unreadable."
            ) from None
        except pd.errors.ParserError as e:
            logging.error(f"Malformed CSV file at {self.contract_path}: {e}")
            raise ValueError(f"Malformed CSV file at {self.contract_path}: {e}") from e
        ...
        symbol_dict: dict[str, str] = dict(
            zip(contracts_df["dataSymbol"], contracts_df["instrumentType"], strict=False)
        )
