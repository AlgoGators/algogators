...
class Loader(ABC):
    ...
    def validate_data_quality(self, db_conn) -> bool:
        ...
        try:
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM symbols WHERE symbol IS NULL OR asset_type IS NULL"
            )
            null_count = cursor.fetchone()[0]
            if null_count > 0:
                self.logger.error(
                    f"Data quality check failed: {null_count} rows have NULL values in symbol or asset_type columns."
                )
                return False
            ...
            if zero_count > 0:
                self.logger.error(
                    f"Data quality check failed: {zero_count} rows have zero values in value_column."
                )
                return False
