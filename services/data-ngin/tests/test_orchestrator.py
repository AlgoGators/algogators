...
from data_ngin.domain.ports import CleanerPort, FetcherPort, SymbolSourcePort
...
class TestOrchestrator(unittest.IsolatedAsyncioTestCase):
    ...
    def test_orchestrator_initialization(self, mock_get_instance: MagicMock) -> None:
        ...
        mock_get_instance.assert_any_call(
            self.mock_config, "loader", "class", expected_port=SymbolSourcePort
        )
        mock_get_instance.assert_any_call(
            self.mock_config, "fetcher", "class", expected_port=FetcherPort
        )
        mock_get_instance.assert_any_call(
            self.mock_config, "cleaner", "class", expected_port=CleanerPort
        )

    @patch(
        "data_ngin.application.orchestrator.determine_date_range",
        return_value=("2023-01-01", "2023-01-02"),
    )
    @patch(
        "data_ngin.application.orchestrator.Orchestrator.retrieve_and_process_data",
    ...
    @patch("data_ngin.application.orchestrator.get_instance")
    def test_get_symbols_and_date_range(
    ...
    @patch("data_ngin.application.orchestrator.determine_date_range")
    @patch("data_ngin.application.orchestrator.get_instance")
