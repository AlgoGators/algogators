...
class TestGraphQLServer(unittest.TestCase):
    ...
    def _client(self):
        patch("data_ngin.api.server.load_config", return_value={}).start()
        patch("data_ngin.api.server.OhlcvRepository", return_value=self.mock_repository).start()
        self.addCleanup(patch.stopall)
        from data_ngin.api.server import app
    ...
    def test_ohlcv_query_maps_rows_to_bars(self) -> None:
        ...
        self.mock_repository.get_ohlcv_data.assert_called_once_with(
            "2023-01-01", "2023-01-02", symbols=["ES"]
        )
...
class TestApiKeyAuth(unittest.TestCase):
    ...
    def test_wrong_key_rejected(self) -> None:
        response = self.client.post(
            "/graphql", json={"query": "{ symbols }"}, headers={"X-API-Key": "wrong"}
        )
        self.assertEqual(response.status_code, 401)
        self.mock_repository.connect.assert_not_called()
    ...
    def test_correct_key_allowed(self) -> None:
        ...
        response = self.client.post(
            "/graphql", json={"query": "{ symbols }"}, headers={"X-API-Key": "secret123"}
        )
        self.assertEqual(response.status_code, 200)
        self.mock_repository.connect.assert_called_once()
...
class TestRateLimiting(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        # instead of `importlib.reload`-ing data_ngin.api.server: reload rebinds
        # the module object itself, which is shared with TestGraphQLServer
        # and TestApiKeyAuth (all three `from data_ngin.api.server import app` the
        ...
        from data_ngin.api.server import app, rate_limiter
