...
class TestIntegrationPipeline(unittest.IsolatedAsyncioTestCase):
    ...
    @patch(
        "data_ngin.infrastructure.loader.csv_loader.CSVLoader.load_symbols",
        return_value={"ES": "FUTURE", "NQ": "FUTURE"},
    )
    @patch("data_ngin.infrastructure.fetcher.databento_fetcher.db.Historical")
    @patch(
        "data_ngin.infrastructure.fetcher.databento_fetcher.DatabentoFetcher.fetch_data",
        new_callable=AsyncMock,
    )
    @patch("data_ngin.infrastructure.cleaner.databento_cleaner.DatabentoCleaner.clean")
    @patch("data_ngin.infrastructure.repository.ohlcv_repository.OhlcvRepository.connect")
    @patch("data_ngin.infrastructure.repository.ohlcv_repository.OhlcvRepository.close")
    @patch("data_ngin.infrastructure.repository.ohlcv_repository.OhlcvRepository.insert_data")
